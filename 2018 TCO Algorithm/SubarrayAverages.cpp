...
{
    vector<double> arr;
    vector<double> findBest(vector<int> _arr)
    {
        int n = _arr.size();
        rep(i, n) arr.push_back(_arr[i]);
        rep(i, n)
        {
            double sml = arr[i];
            int to = i;
...
bool double_vector_equal(const vector<double> &expected, const vector<double> &received)
{
    ...
    int n = expected.size();
    for (int i = 0; i < n; ++i)
        if (!double_equal(expected[i], received[i]))
            return false;
    return true;
}
...
int main(int argc, char *argv[])
{
    ...
    if (mainProcess)
    {
        cout << "SubarrayAverages (250 Points)" << endl
             << endl;
    }
